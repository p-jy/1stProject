...
public class LibraryProgram implements ConsoleProgram {
	...
	private void rentBook() {
	    ...
	    System.out.print("대여할 도서 번호를 선택하세요: ");
	    int n = scan.nextInt();
	    scan.nextLine();
	    ...
	    Book selectedBook = bookList.get(n - 1);
	    
	    
	    boolean result = RentReturn.rentBook(user, selectedBook);
	    if (result) {
	...
	private void returnBook() {
	    ...
	    List<RentReturn> rentList = user.getRentList();
	    ...
	    for (int i = 0; i < rentList.size(); i++) {
	        ...
	        System.out.println((i + 1) + ". " + rentList.get(i).getBook());
	    }
	    
	  
	    System.out.print("반납할 도서 번호를 선택하세요: ");
	    int n = scan.nextInt();
	    scan.nextLine();
	    if (n < 1 || n > rentList.size()) {
	        System.out.println("잘못된 선택입니다.");
	        return;
	    }
	    
	    RentReturn selectedRent = rentList.get(n - 1);
	    Book selectedBook = selectedRent.getBook();
	    
	    
	    boolean result = RentReturn.returnBook(user, selectedBook);
	    if (result) {
